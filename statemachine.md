...
| TopOfClimbPilot | `Settings: TopOfClimbPilot enabled` AND `Altitude >= SimBrief initial altitude` AND `IsOnGround=False` AND `VerticalSpeed > 0` | Play once when reaching planned cruise altitude (disabled by default, simplistic - works best with single altitude plans) |
...
| TopOfDescentPilot | `Settings: TopOfDescentPilot enabled` AND `Altitude <= SimBrief initial altitude` AND `IsOnGround=False` AND `VerticalSpeed < 0` | Play once when descending through planned cruise altitude (disabled by default, simplistic - works best with single altitude plans) |
